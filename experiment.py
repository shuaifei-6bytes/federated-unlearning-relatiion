...
核心假设：Feature ≠ Relation
  同一个特征（水背景）同时参与两条语义关系：
    1. 水背景 → 水鸟类别   （伪关联 / 错误关系，需要删除）
    2. 水背景 → 环境类别   （合理关系，需要保留）
  Feature-level Unlearning 只能整体削弱"水背景"这个特征的影响，
  无法区分其中某一条关系，因此会造成过度遗忘（Over-unlearning）。

三种对比方法：
  M_global   : FedAvg 正常联邦训练得到的原始模型（遗忘前基线）
  M_feature  : Ferrari 式 feature-level unlearning（最小化 feature sensitivity，
               即对输入加高斯噪声扰动，最小化模型输出对扰动的敏感度）
  M_relation : relation-level unlearning（冻结 backbone 与环境 head，
               只用去关联的均衡数据重训鸟分类 head，仅删除"背景→鸟类别"关系）

评价指标：
  1. Background Gap      —— 目标关系遗忘能力（越低越好）
  2. Environment Accuracy—— 非目标关系保持能力（M_relation 应保持，M_feature 应明显下降）
  3. Bird Accuracy       —— 正常分类能力保持

用法示例：
  # 完整实验（Colab/Kaggle GPU）
  python experiment.py --data_dir ./data --global_epochs 20 --unlearn_epochs 5

  # 快速冒烟（小样本、少轮次，验证流程可跑通）
...
def load_metadata(root):
    ...
    本实验不依赖具体编码，只用 y==place 判断 aligned / conflicting 组。
    """
...
class DualHeadResNet(nn.Module):
    def __init__(self, num_bird=2, num_env=2, pretrained=True):
        ...
        # 去掉 fc，保留到 avgpool，输出 (B, 512, 1, 1)
        self.features = nn.Sequential(*list(backbone.children())[:-1])
        self.head_bird = nn.Linear(512, num_bird)   # 任务1：鸟类别（主要任务）
        self.head_env = nn.Linear(512, num_env)     # 任务2：环境类别（辅助任务）
...
def evaluate(model, loader, device):
    """返回 (bird_acc, env_acc, gap, 各分组 bird_acc 字典)。

    gap = aligned 组鸟分类准确率 - conflicting 组鸟分类准确率。
    aligned(y==place) 是"背景与鸟类别一致"的样本（伪关联所在），
    conflicting(y!=place) 是"背景与鸟类别矛盾"的样本。
    模型越依赖背景（伪关联越强），gap 越大。
    """
    model.eval()
...
def train_fedavg(model, client_loaders, device, global_epochs, local_epochs, lr, frac, seed):
    """FedAvg 联邦训练：多任务（鸟分类 + 环境分类）联合，得到 M_global。"""
...
def feature_unlearn(model, unlearn_loader, device, epochs, lr, sigma):
    ...
    核心：对输入 x 加高斯噪声扰动 δ，最小化 feature sensitivity
        s = E[ ||f(x) - f(x + δ)||_2 / ||δ||_2 ]
    使模型对"背景"这个特征的扰动不再敏感 —— 等价于整体削弱背景特征影响。
    由于 Waterbirds 无像素级 mask，δ 作用于整张图（背景占图的大部分），
    这正是 Ferrari 在无局部 mask 场景下的标准应用形式。
    ...
    for ep in range(epochs):
        ...
        for x, y, place in unlearn_loader:
            x = x.to(device)
            delta = torch.randn_like(x) * sigma   # 高斯扰动，作用整个输入（背景特征）
            optimizer.zero_grad()
            lb1, le1 = model(x)
...
def build_balanced_loader(dataset, indices, batch_size, seed, num_workers=0):
    """从给定 indices 中构造四组（aligned/conflicting 各半）均衡子集，
...
def relation_unlearn(model, unlearn_dataset, unlearn_indices, device, epochs, lr, batch_size, seed):
    """relation-level unlearning（本文方法）。

    冻结 backbone 与 head_env，只用均衡数据重训 head_bird：
    让鸟分类 head 学会"不看背景、只看鸟主体"，从而只删除
    "背景→鸟类别"这条伪关联，同时完整保留"背景→环境"关系。
    """
    freeze_except_head_bird(model)
    loader = build_balanced_loader(unlearn_dataset, unlearn_indices, batch_size, seed)
...
# 联邦数据划分
# ----------------------------------------------------------------------------
def build_client_loaders(train_dataset, num_clients, batch_size, seed, num_workers=0):
    """把训练集随机切分给 num_clients 个客户端（保持原分布）。"""
    indices = list(range(len(train_dataset)))
    random.seed(seed)
    random.shuffle(indices)
    loaders = []
    per = len(indices) // num_clients
    for i in range(num_clients):
        part = indices[i * per:(i + 1) * per] if i < num_clients - 1 else indices[i * per:]
        sub = Subset(train_dataset, part)
        loaders.append(DataLoader(sub, batch_size=batch_size, shuffle=True, num_workers=num_workers))
    return loaders
...
def main():
    ...
    parser.add_argument("--unlearn_epochs", type=int, default=5, help="feature/relation 遗忘阶段的轮数")
    ...
    client_loaders = build_client_loaders(train_ds, args.num_clients, args.batch_size, args.seed, num_workers)

    test_indices = list(range(len(test_ds)))
    if args.max_test_samples:
        test_indices = test_indices[:args.max_test_samples]
    test_loader = DataLoader(Subset(test_ds, test_indices), batch_size=args.batch_size, shuffle=False, num_workers=num_workers)

    # 遗忘客户端 = 客户端 0 的数据（模拟提出遗忘请求的客户端 A）
    unlearn_dataset = train_ds
    unlearn_indices = list(range(len(train_ds)))[:len(train_ds) // args.num_clients] if not args.max_train_samples \
        else train_indices[:len(train_indices) // args.num_clients]
    unlearn_loader = DataLoader(Subset(train_ds, unlearn_indices), batch_size=args.batch_size, shuffle=True, num_workers=num_workers)
    ...
    results["M_global"] = {"bird_acc": r[0], "env_acc": r[1], "gap": r[2],
                           "aligned_acc": r[3]["aligned"], "conflicting_acc": r[3]["conflicting"]}
    print(f"  M_global  -> bird_acc={r[0]:.4f}  env_acc={r[1]:.4f}  gap={r[2]:.4f}")
    ...
    results["M_feature"] = {"bird_acc": r[0], "env_acc": r[1], "gap": r[2],
                            "aligned_acc": r[3]["aligned"], "conflicting_acc": r[3]["conflicting"]}
    print(f"  M_feature -> bird_acc={r[0]:.4f}  env_acc={r[1]:.4f}  gap={r[2]:.4f}")
    ...
    relation_unlearn(model_relation, unlearn_dataset, unlearn_indices, device,
                     args.unlearn_epochs, args.unlearn_lr, args.batch_size, args.seed)
    r = evaluate(model_relation, test_loader, device)
    results["M_relation"] = {"bird_acc": r[0], "env_acc": r[1], "gap": r[2],
                             "aligned_acc": r[3]["aligned"], "conflicting_acc": r[3]["conflicting"]}
    print(f"  M_relation-> bird_acc={r[0]:.4f}  env_acc={r[1]:.4f}  gap={r[2]:.4f}")
    ...
    with open(os.path.join(args.out_dir, "comparison_table.csv"), "w", newline="") as f:
        ...
        w.writerow(["方法", "Bird Accuracy(鸟分类保持)", "Environment Accuracy(环境关系保持)",
                    "Background Gap(目标关系遗忘, 越低越好)"])
        for name, d in results.items():
            w.writerow([name, f"{d['bird_acc']:.4f}", f"{d['env_acc']:.4f}", f"{d['gap']:.4f}"])

    print("\n===== 结果汇总 =====")
    print(f"{'方法':<12} {'鸟分类acc':<10} {'环境acc':<10} {'背景gap':<10}")
    for name, d in results.items():
